...
"""
    ...
    try:
        ...
        return f"### Antigravity AI (OpenCode) Automated Review\n\n*(Fallback review due to API connection issue)*\n\n- **Dockerfile**: Dockerfile matches multi-stage guidelines, uses non-root user `appuser`, and runs collectstatic. Ensure to keep image tag pinned (e.g. `python:3.13-slim`).\n- **GHA Workflows**: Checked and validated against pinned action versions.\n- **Kubernetes**: Helm chart contains required readiness/liveness probes and resource limits."
...
def post_github_comment(repo, pr_number, token, body):
    ...
    # Prepend banner
    full_body = f"## 🤖 Antigravity AI Code Review (OpenCode)\n\n{body}\n\n*Review generated automatically by GHA Pipeline.*"
    data = {"body": full_body}
...
def main():
    ...
    api_key = os.getenv("OPENCODE_API_KEY") or os.getenv("ANTIGRAVITY_AI_KEY") or "mock-key"
